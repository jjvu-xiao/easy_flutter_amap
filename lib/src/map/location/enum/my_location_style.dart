...
class MyLocationStyle {
  ...
  late final String myLocationIcon;

  /// 锚点横坐标方向的偏移量 [Android]
  late final double anchorU;

  /// 锚点纵坐标方向的偏移量 [Android]
  late final double anchorV;

  /// 圆形区域（以定位位置为圆心，定位半径的圆形区域）的填充颜色值 [Android, iOS]
  late final Color radiusFillColor;
  ...
  /// 是否显示定位小蓝点 [Android]
  late final bool showMyLocation;

  /// 精度圈是否显示，默认YES [iOS]
  late final bool showsAccuracyRing;

  /// 是否显示方向指示(MAUserTrackingModeFollowWithHeading模式开启)。默认为YES [iOS]
  late final bool showsHeadingIndicator;

  /// 定位点背景色，不设置默认白色 [iOS]
  late final Color locationDotBgColor;

  /// 定位点蓝色圆点颜色，不设置默认蓝色 [iOS]
  late final Color locationDotFillColor;

  /// 内部蓝色圆点是否使用律动效果, 默认YES [iOS]
  late final bool enablePulseAnimation;
